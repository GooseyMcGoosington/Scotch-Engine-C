...
void R_DRAW_ENTITIES(Uint8 *pixels, Level *level, player character, float pCs, float pSn) {
    float px = character.x; 
    float py = character.y; 
    float pz = character.z;
    float pitch = 0; //character.pitch;
    float shear = tanf(pitch);
    float focalShear = character.focalLength * shear;

    for (int e=0; e<amnt_entities; e++) {
        entity Entity = entities[e];
        sector *Sector = level->sectors[Entity.oid];
        float wz1 = (Sector->elevation+pz);
        // We can look for portal cull bounds.
        // If its -1, it is uninitialised by R_DRAW_SECTOR, and we can skip.
        cull bounds = cullBounds[Entity.oid];
        portalCull portalBounds = {bounds.x0, bounds.x1, bounds.y0, bounds.y1, bounds.y2, bounds.y3, bounds.x1-bounds.x0};

        if (bounds.x0 == -1 || bounds.x1 == -1 || bounds.y0 == -1 || bounds.y1 == -1 || bounds.y2 == -1 || bounds.y3 == -1) {
            continue;
        }
        ...
        if (!Entity.animated) {
            tfile tex = texture_list->files[Entity.tid+entities[e].a ];
...
void R_drawSector(Uint8 *pixels, Level *level, sector *Sector, player character, float pSn, float pCs, portalCull portalBounds, int doClip) {
    ...
    for (int portalIndex=0; portalIndex<portalCount; portalIndex++) {
        ...
        cullBounds[portalInfo.sector_link].x0 = portalInfo.portalBounds.x0;
        cullBounds[portalInfo.sector_link].x1 = portalInfo.portalBounds.x1;
        cullBounds[portalInfo.sector_link].y0 = portalInfo.portalBounds.y0;
        cullBounds[portalInfo.sector_link].y1 = portalInfo.portalBounds.y1;
        cullBounds[portalInfo.sector_link].y2 = portalInfo.portalBounds.y2;
        cullBounds[portalInfo.sector_link].y3 = portalInfo.portalBounds.y3;
...
void R_startDrawSector(Uint8 *pixels, Level *level, sector *Sector, player character, float pSn, float pCs, portalCull portalBounds, int origin) {
    ...
    memset(cullBounds, -1, sizeof(cull)*MAX_SECTORS);
    cullBounds[origin].x0 = portalBounds.x0;
    cullBounds[origin].x1 = portalBounds.x1;
    cullBounds[origin].y0 = portalBounds.y0;
    cullBounds[origin].y1 = portalBounds.y1;
    cullBounds[origin].y2 = portalBounds.y2;
    cullBounds[origin].y3 = portalBounds.y3;
